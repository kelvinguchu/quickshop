...
export function CheckoutForm() {
  ...
  // Convert total to KES (assuming 1 USD = 128 KES)
  const amountInKES = Math.round(total * 128)
  ...
  useEffect(() => {
    ...
    button.setAttribute('data-country', (formData.country || 'Kenya').substring(0, 2).toUpperCase())
    ...
    intaSend
      .on('COMPLETE', async (results: any) => {
        ...
        try {
          ...
          setPaymentError('Payment received but failed to create order. Please contact support.')
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="bg-white rounded-sm border border-[#e0d8c9] p-4">
        <h2 className="font-cinzel text-lg text-[#382f21] mb-4">Customer Info</h2>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label htmlFor="firstName" className="block text-xs text-[#8a7d65] mb-1">
              First Name*
            </label>
            <input
              type="text"
          ...
          <div>
            <label htmlFor="lastName" className="block text-xs text-[#8a7d65] mb-1">
              Last Name*
            </label>
            <input
              type="text"
      ...
      <div className="bg-white rounded-sm border border-[#e0d8c9] p-4">
        <h2 className="font-cinzel text-lg text-[#382f21] mb-4">Shipping Address</h2>

        <div className="space-y-4">
          <div>
            <label htmlFor="address" className="block text-xs text-[#8a7d65] mb-1">
              Address*
            </label>
            <input
              type="text"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label htmlFor="city" className="block text-xs text-[#8a7d65] mb-1">
                City*
              </label>
              <input
                type="text"
            ...
            <div>
              <label htmlFor="postalCode" className="block text-xs text-[#8a7d65] mb-1">
                Postal Code*
              </label>
              <input
                type="text"
            ...
            <div>
              <label htmlFor="country" className="block text-xs text-[#8a7d65] mb-1">
                Country*
              </label>
              <select
                id="country"
      ...
      <div className="bg-white rounded-sm border border-[#e0d8c9] p-4">
        ...
        <p className="text-sm text-[#8a7d65] mb-4">
          Total Amount: KES {amountInKES.toFixed(2)} (${finalTotal.toFixed(2)})
        </p>
        ...
        {isProcessing ? (
          <div className="text-center py-4">
            ...
            <p className="mt-2 text-sm text-[#8a7d65]">Processing your payment...</p>
          </div>
        ) : (
          <div ref={paymentButtonRef} className="w-full">
            {!isScriptLoaded && (
              <div className="text-center py-4">
                <div className="inline-block h-6 w-6 animate-spin rounded-full border-2 border-[#8a7d65] border-t-transparent"></div>
                <p className="mt-2 text-sm text-[#8a7d65]">Loading payment options...</p>
              </div>
            )}
          </div>
        )}
