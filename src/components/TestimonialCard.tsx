...
export default function TestimonialCard({ testimonial }: TestimonialCardProps) {
  ...
  const ratingNumber =
    typeof testimonial.rating === 'string' ? parseInt(testimonial.rating, 10) : testimonial.rating
    ...
    <div className="bg-white p-8 rounded-sm shadow-sm relative flex flex-col h-full">
      ...
      <div className="flex-grow">
        <div className="text-[#d4af37] mb-4">
          {[...Array(5)].map((_, i) => (
            <span key={i} className={i < ratingNumber ? 'text-[#d4af37]' : 'text-[#e6ded0]'}>
              ★
            </span>
          ))}
        </div>
      ...
      <div className="flex items-center mt-4">
        ...
        <span className="font-cinzel text-[#382f21] font-medium">{testimonial.customerName}</span>
      </div>
    </div>
  )
}
