    ...
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      >
        <SheetHeader className="border-b border-[#e0d8c9]/40 py-3">
          <SheetTitle className="font-cinzel text-lg text-[#382f21] px-4">Shopping Cart</SheetTitle>
        </SheetHeader>
        ...
        <div className="flex-1 overflow-y-auto">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-64 text-center p-6">
              ...
              <h3 className="font-cinzel text-lg text-[#382f21] mb-2">Your cart is empty</h3>
              <p className="text-[#8a7d65] text-sm">Items added to your cart will appear here</p>
            </div>
          ) : (
        ...
        {items.length > 0 && (
          <div className="border-t border-[#e0d8c9]/40 py-4 px-4 flex flex-col gap-3">
            <div className="flex justify-between items-center">
              <span className="font-cinzel text-base text-[#382f21]">Total</span>
              <div>
                <span className="font-cinzel text-lg font-semibold text-[#382f21]">
                  ${total.toFixed(2)}
                </span>
                <p className="text-xs text-[#8a7d65] text-right">Shipping & taxes at checkout</p>
              </div>
            </div>
