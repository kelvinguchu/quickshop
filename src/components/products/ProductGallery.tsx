...
const ProductGallery: React.FC<ProductGalleryProps> = ({
  ...
  const [activeImageUrl, setActiveImageUrl] = useState<string | null>(mainImage?.url || null)
  ...
  const displayImages = React.useMemo(() => {
    ...
    if (selectedVariation) {
      const variationImages = [
        { url: selectedVariation.image.url, alt: `${productName} - ${selectedVariation.color}` },
      ]

      // Add additional images for this color variation if they exist
      if (selectedVariation.additionalImages && selectedVariation.additionalImages.length > 0) {
